<template>
  <transition name="fade">
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" @click.self="$emit('close')">
      <div class="w-[92%] max-w-lg rounded-2xl border border-slate-100 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-800">
        <div class="flex items-center justify-between mb-5">
          <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100">Import Users</h3>
        ...
        <div class="mb-4 rounded-xl border border-indigo-100 bg-indigo-50 p-4 text-sm text-indigo-700 dark:border-indigo-900/50 dark:bg-indigo-950/30 dark:text-indigo-400">
          ...
          <p class="mt-1 text-indigo-600 dark:text-indigo-400">Upload an .xlsx file with columns: First Name, Last Name, Email, Role</p>
          <a :href="templateUrl" class="mt-2 inline-flex items-center gap-1.5 font-semibold text-indigo-700 hover:text-indigo-900 underline dark:text-indigo-400 dark:hover:text-indigo-300">
            Download template
          </a>
        </div>
        ...
        >
          ...
          <p class="text-sm font-semibold text-slate-700 dark:text-slate-300">{{ file ? file.name : 'Drop file here or click to browse' }}</p>
          <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">.xlsx, .xls, or .csv</p>
        </div>

        <div v-if="result" class="mt-4 space-y-2 rounded-xl border p-4" :class="result.success_count > 0 && result.failed_count === 0 ? 'border-emerald-100 bg-emerald-50 dark:border-emerald-900/50 dark:bg-emerald-950/30' : 'border-amber-100 bg-amber-50 dark:border-amber-900/50 dark:bg-amber-950/30'">
          <p class="text-sm font-bold" :class="result.failed_count > 0 ? 'text-amber-800 dark:text-amber-300' : 'text-emerald-800 dark:text-emerald-300'">
            Import Complete
          </p>
          <p class="text-sm" :class="result.failed_count > 0 ? 'text-amber-700 dark:text-amber-400' : 'text-emerald-700 dark:text-emerald-400'">
          ...
          <div v-if="result.failed_rows && result.failed_rows.length > 0" class="mt-2 max-h-32 overflow-y-auto space-y-1">
            <div v-for="fr in result.failed_rows" :key="fr.row" class="text-xs text-amber-600 dark:text-amber-400">
