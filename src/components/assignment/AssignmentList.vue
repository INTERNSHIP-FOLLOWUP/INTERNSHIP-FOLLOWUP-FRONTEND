                  ...
                  <p class="mt-1 text-xs text-slate-400 dark:text-slate-500">
                    {{
                      hasActiveFilters
                        ...
                        : 'No internship assignments have been created yet.'
                    }}
                  </p>
                </div>
              </td>
...
const activeCount = computed(
  () => store.assignments.filter((a: Assignment) => a.status === 'Assigned').length,
)
const pendingCount = computed(
  () => store.assignments.filter((a: Assignment) => a.status === 'In Progress').length,
)
const completedCount = computed(
  () => store.assignments.filter((a: Assignment) => a.status === 'Completed').length,
)
