  ...
  >
    <transition-group name="toast" tag="div" class="flex w-full max-w-sm flex-col gap-3">
      <div
        ...
        :enter="{ opacity: 1, x: 0, scale: 1, transition: { type: 'spring', stiffness: 300, damping: 24 } }"
        :leave="{ opacity: 0, x: 40, scale: 0.92, transition: { type: 'spring', stiffness: 300, damping: 24 } }"
...
function createIcon(d: string, color: string) {
  return defineComponent({
    setup() {
      return () =>
        h('svg', { class: `h-5 w-5 ${color}`, fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' }, [
...
function iconComponent(type: ToastType) {
  switch (type) {
    case 'success': return createIcon('M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', 'text-emerald-500')
    case 'error': return createIcon('M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z', 'text-red-500')
    case 'warning': return createIcon('M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z', 'text-amber-500')
    case 'info': return createIcon('M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z', 'text-blue-500')
  }
}
</script>
