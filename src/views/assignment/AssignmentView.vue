    ...
    <template v-if="isStudent">
      ...
      <div class="rounded-2xl border border-slate-100 bg-white shadow-sm">
        ...
        <div v-else-if="!myInternship" class="flex flex-col items-center justify-center py-16 px-4 text-center">
          ...
          <h3 class="text-base font-semibold text-slate-800">No internship assignment yet</h3>
          <p class="mt-2 max-w-sm text-sm text-slate-500">
            Your internship assignment details will appear here once your tutor or admin assigns you to a company.
          </p>
        </div>
        ...
        <div v-else class="p-6 space-y-6">
          ...
          <div class="flex items-center gap-3 rounded-xl px-4 py-3" :class="statusBannerClass(myInternship.status)">
            <span class="flex h-3 w-3 rounded-full" :class="statusDotClass(myInternship.status)" />
            <div>
              ...
              <p class="text-xs" :class="statusBannerSubtextClass(myInternship.status)">
                <template v-if="myInternship.status === 'Assigned'">Waiting to start</template>
                <template v-else-if="myInternship.status === 'In Progress'">Currently active</template>
                <template v-else-if="myInternship.status === 'Completed'">Successfully completed</template>
                <template v-else>Internship has ended</template>
              </p>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div class="rounded-xl border border-slate-100 bg-slate-50/50 p-4 transition-colors hover:bg-slate-50">
              <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Company</p>
              <p class="text-sm font-semibold text-slate-800">{{ myInternship.company_name }}</p>
            </div>
            <div class="rounded-xl border border-slate-100 bg-slate-50/50 p-4 transition-colors hover:bg-slate-50">
              <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Position</p>
              <p class="text-sm font-semibold text-slate-800">{{ myInternship.position }}</p>
            </div>
            <div class="rounded-xl border border-slate-100 bg-slate-50/50 p-4 transition-colors hover:bg-slate-50">
              <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Tutor</p>
              <p class="text-sm font-semibold text-slate-800">{{ myInternship.tutor_name }}</p>
            </div>
            <div class="rounded-xl border border-slate-100 bg-slate-50/50 p-4 transition-colors hover:bg-slate-50">
              <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Duration</p>
