  ...
  <div class="space-y-6" @click="openKebabId = null">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          @click="exportExcel(selectedBatch)"
          :disabled="!selectedBatch"
    ...
    <div class="rounded-2xl border border-slate-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-3 dark:border-slate-800">
        <div class="flex flex-wrap items-center gap-3 flex-1">
          ...
          >
            ...
            <option value="has_students">With Students</option>
            <option value="empty">Empty</option>
          </select>

          <button
            v-if="searchQuery || selectedYear || selectedStatus"
            @click="clearFilters"
            class="h-9 px-3 text-xs font-semibold text-slate-500 hover:text-slate-800 transition-colors"
          >
            Clear Filters
          </button>
        </div>
      ...
      <div v-else-if="store.error" class="flex flex-col items-center justify-center py-16 text-center px-4">
        ...
        <button @click="fetchBatches" class="mt-3 rounded-lg bg-indigo-50 px-3.5 py-2 text-xs font-semibold text-indigo-600 hover:bg-indigo-100 transition-colors">
          Try Again
        </button>
      </div>

      <div v-else-if="filteredBatches.length === 0">
        <div v-if="searchQuery || selectedYear || selectedStatus" class="flex flex-col items-center justify-center py-16 text-center px-4">
          <svg class="h-10 w-10 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <p class="mt-3 text-sm font-semibold text-slate-400">No batches match active filters.</p>
        ...
        <div v-else class="flex flex-col items-center justify-center py-16 text-center px-4">
          ...
          <button @click="openCreate" class="mt-4 inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-all hover:bg-indigo-700 active:scale-95">
            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-else>
              ...
              >
                <td class="whitespace-nowrap px-5 py-4">
                  <div class="flex items-center gap-3">
                    <div class="flex h-8 w-8 items-center justify-center rounded-lg bg-indigo-50 text-xs font-bold text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400">
                      {{ getInitials(batch.batch_name) }}
                    </div>
                    <router-link
                      :to="`/admin/batches/${batch.id}`"
                      @click.stop
                      class="font-semibold text-slate-900 hover:text-indigo-600 transition-colors dark:text-white dark:hover:text-indigo-400"
                    >
                      {{ batch.batch_name }}
                    </router-link>
                  </div>
                ...
                <td class="whitespace-nowrap px-5 py-4">
                  <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-bold" :class="countBadgeClass(batch.students_count)">
                  ...
                  <div class="relative inline-block text-center">
                    ...
                    <transition name="fade">
                      ...
                      >
                        ...
                        <button
                          @click.stop="openEdit(batch); openKebabId = null"
                          class="flex w-full items-center gap-2.5 px-3.5 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 hover:text-indigo-600 transition-colors dark:text-slate-200 dark:hover:bg-slate-700"
                        >
                          <svg class="h-4 w-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button
                          @click.stop="deleteBatch(batch.id); openKebabId = null"
        ...
        <div class="divide-y divide-slate-100 md:hidden dark:divide-slate-800">
          <div
            ...
            :key="batch.id"
            @click="goToBatch(batch.id)"
            class="px-4 py-4 cursor-pointer transition-colors hover:bg-slate-50/30 dark:hover:bg-slate-800/30"
          >
            <div class="flex items-start justify-between gap-3">
              <div class="flex items-center gap-3 min-w-0">
                <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-indigo-50 text-xs font-bold text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400">
                  {{ getInitials(batch.batch_name) }}
                </div>
                <div class="min-w-0">
              ...
              <span class="shrink-0 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-bold" :class="countBadgeClass(batch.students_count)">
                <span class="h-1.5 w-1.5 rounded-full" :class="countDotClass(batch.students_count)" />
            ...
            <div class="mt-3 flex items-center gap-2">
              <router-link
                :to="`/admin/batches/${batch.id}`"
                @click.stop
                class="flex-1 text-center rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:text-slate-400 dark:hover:bg-slate-800"
              >
                View Details
              </router-link>
              <button
                @click.stop="openEdit(batch)"
              ...
              <button
                ...
                class="rounded-lg border border-red-200 px-3 py-1.5 text-xs font-semibold text-red-600 transition-colors hover:bg-red-50 dark:border-red-900/30 dark:text-red-400 dark:hover:bg-red-900/20"
              >
                Delete
              </button>
            </div>
...
const availableYears = computed(() => {
  const years = new Set(storeBatches.value.map((b) => b.year).filter(Boolean))
  return Array.from(years).sort((a, b) => Number(b) - Number(a))
})

const filteredBatches = computed(() => {
  let list = storeBatches.value
  if (selectedYear.value) {
    list = list.filter((b) => String(b.year) === String(selectedYear.value))
  }
  if (selectedStatus.value === 'has_students') {
    list = list.filter((b) => (b.students_count || 0) > 0)
  } else if (selectedStatus.value === 'empty') {
    list = list.filter((b) => (b.students_count || 0) === 0)
  }
  const q = searchQuery.value.toLowerCase().trim()
  if (q) {
    list = list.filter((b) => b.batch_name.toLowerCase().includes(q) || b.year.toString().includes(q))
  }
  return list
})
