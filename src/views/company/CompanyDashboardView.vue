<template>
  <div class="animate-fade-in">
      ...
      <div v-for="stat in stats" :key="stat.label"
        ...
        <div class="mt-3 flex items-center gap-1.5">
          <span class="inline-flex items-center gap-0.5 rounded-full px-2 py-0.5 text-xs font-medium"
            ...
            <svg v-if="stat.trend.startsWith('+')" class="h-3 w-3" fill="none" stroke="currentColor"
              viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 10l7-7m0 0l7 7m-7-7v18" />
            </svg>
            <svg v-else-if="stat.trend.startsWith('-')" class="h-3 w-3" fill="none" stroke="currentColor"
              viewBox="0 0 24 24">
    ...
    <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
      <!-- Company Supervision Queue -->
      <div class="lg:col-span-2 rounded-2xl border border-gray-100 bg-white shadow-sm">
        <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4">
          <h2 class="text-base font-semibold text-gray-900">Supervision Overview</h2>
          <button class="text-sm font-medium text-primary-600 transition-colors hover:text-primary-700" type="button">
            View students
          </button>
        </div>

        <div class="divide-y divide-gray-50">
          <div v-for="student in supervisionQueue" :key="student.id"
            ...
            <div class="min-w-0 flex-1">
              <p class="text-sm font-medium text-gray-900">
                {{ student.name }}
                <span class="font-normal text-gray-500">{{ student.stage }}</span>
              </p>
              <p class="mt-0.5 text-sm text-gray-400">{{ student.latestUpdate }}</p>
            </div>
            <div class="shrink-0 text-xs text-gray-400">{{ student.lastCheckIn }}</div>
          </div>

          <!-- Empty state -->
          <div v-if="supervisionQueue.length === 0" class="flex flex-col items-center py-12 text-center">
            ...
            <h3 class="mt-4 text-sm font-semibold text-gray-700">No supervision items</h3>
            <p class="mt-1 text-xs text-gray-400">
              Students assigned to your company will appear here for evaluation and feedback.
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-gray-100 bg-white shadow-sm">
          <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4">
            <h2 class="text-base font-semibold text-gray-900">Pending Evaluations</h2>
            <button class="text-sm font-medium text-primary-600 transition-colors hover:text-primary-700" type="button">
              Review now
            </button>
          </div>
          ...
          <div class="divide-y divide-gray-50">
            <div v-for="review in pendingEvaluations" :key="review.id"
              class="flex items-start gap-3 px-6 py-4 transition-colors hover:bg-gray-50/50">
              <div class="flex h-2 w-2 mt-1.5 shrink-0 rounded-full"
                :class="review.priority === 'high' ? 'bg-rose-500' : review.priority === 'medium' ? 'bg-amber-500' : 'bg-emerald-500'" />
              <div class="min-w-0 flex-1">
                <p class="text-sm font-medium text-gray-900">{{ review.student }}</p>
                <p class="mt-0.5 text-xs text-gray-400">{{ review.template }}</p>
                <p class="mt-1 text-xs font-medium" :class="review.dueClass">{{ review.due }}</p>
              </div>
            </div>

            <!-- Empty state -->
            <div v-if="pendingEvaluations.length === 0" class="flex flex-col items-center py-10 text-center">
              ...
              <h3 class="mt-3 text-sm font-semibold text-gray-700">All evaluations are up to date</h3>
              <p class="mt-1 text-xs text-gray-400">You have no pending reviews right now.</p>
            </div>
          </div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          <h2 class="text-base font-semibold text-gray-900">Company Actions</h2>
    ...
    <div class="mt-6 rounded-2xl border border-gray-100 bg-white shadow-sm">
      <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4">
        ...
        <button class="text-sm font-medium text-primary-600 transition-colors hover:text-primary-700">
          Manage interns
        </button>
      </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm">
          ...
          <tbody class="divide-y divide-gray-50">
            <tr v-for="intern in currentInterns" :key="intern.id" class="transition-colors hover:bg-gray-50/50">
              ...
              <td class="px-6 py-3.5">
                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium" :class="intern.status === 'active'
...
const supervisionQueue = [] as Array<{
  id: number
  name: string
  initials: string
  avatarColor: string
...
const pendingEvaluations = [] as Array<{
  id: number
  student: string
  template: string
  due: string
