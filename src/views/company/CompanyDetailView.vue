<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center gap-3 text-sm">
      ...
      <span class="font-medium text-slate-900">{{ company?.companyName || 'Company Details' }}</span>
    </div>
    ...
    <div v-else-if="error" class="flex flex-col items-center justify-center py-20 text-center">
      ...
      <button @click="fetchData" class="mt-4 rounded-lg bg-indigo-50 px-4 py-2 text-sm font-semibold text-indigo-600 transition-colors hover:bg-indigo-100">
        Try Again
      </button>
    </div>

    <!-- Company Detail Card -->
    <div v-else-if="company" class="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
        ...
        <div class="relative flex flex-col items-center gap-5 sm:flex-row sm:items-end">
          ...
          <div class="text-center sm:text-left">
            ...
            <div class="mt-2 flex flex-wrap items-center justify-center gap-2 sm:justify-start">
              ...
              <span v-if="company.email" class="inline-flex items-center gap-1 rounded-full bg-white/15 px-3 py-1 text-xs font-medium text-white/90 backdrop-blur-sm">
      ...
      <div class="px-8 py-8">
          ...
          <div class="space-y-5">
            ...
            <div v-if="company.email" class="flex items-start gap-3">
              ...
              <div>
                ...
                <a :href="`mailto:${company.email}`" class="text-sm font-semibold text-sky-600 transition-colors hover:text-sky-800">{{ company.email }}</a>
              </div>
            </div>
        ...
        <div class="space-y-5">
          ...
          <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div v-if="company.website" class="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/50 p-4 transition-colors hover:bg-slate-50">
              ...
              <div class="min-w-0">
                ...
                <a :href="normalizeUrl(company.website)" target="_blank" rel="noopener noreferrer" class="block truncate text-sm font-semibold text-violet-600 transition-colors hover:text-violet-800">
                  {{ company.website }}
                </a>
              </div>
            </div>

            <div v-if="company.telegramLink" class="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50/50 p-4 transition-colors hover:bg-slate-50">
              ...
              <div class="min-w-0">
                ...
                <a :href="normalizeUrl(company.telegramLink)" target="_blank" rel="noopener noreferrer" class="block truncate text-sm font-semibold text-sky-600 transition-colors hover:text-sky-800">
          ...
          <div v-if="!company.website && !company.telegramLink" class="rounded-xl border border-dashed border-slate-200 py-6 text-center text-sm text-slate-400">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div v-if="company.createdAt" class="rounded-xl bg-slate-50/60 p-4">
            ...
            <p class="mt-1 text-sm font-semibold text-slate-700">{{ formatDate(company.createdAt) }}</p>
          </div>
          <div v-if="company.updatedAt" class="rounded-xl bg-slate-50/60 p-4">
            <p class="text-xs font-medium text-slate-400">Last Updated</p>
            <p class="mt-1 text-sm font-semibold text-slate-700">{{ formatDate(company.updatedAt) }}</p>
          </div>
