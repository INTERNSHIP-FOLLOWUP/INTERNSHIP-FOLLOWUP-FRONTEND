  ...
  <div>
    <div class="rounded-2xl border dark:border-slate-700 border-gray-100 dark:bg-slate-800 bg-white p-6 shadow-sm">
      <div class="mb-6 flex items-start justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold dark:text-slate-100 text-gray-900">Internship Information</h1>
          <p class="mt-1 text-sm dark:text-slate-400 text-gray-500">View internship details for students assigned to your company.</p>
        </div>
      </div>
      ...
      <div v-if="loading" class="flex items-center justify-center py-12">
        ...
        <span class="ml-3 text-sm dark:text-slate-400 text-gray-500">Loading internship information…</span>
      </div>
      <div
        v-else-if="error"
      ...
      <div v-else class="overflow-x-auto">
              ...
              <td class="px-4 py-3.5">
                <div class="flex items-center gap-3">
                  <UserAvatar
                    ...
                    size="sm"
                  />
                  <span class="font-medium dark:text-slate-100 text-gray-900">{{ item.studentName }}</span>
                </div>
              </td>
              <td class="px-4 py-3.5 dark:text-slate-400 text-gray-600">{{ item.tutorName }}</td>
              <td class="px-4 py-3.5 dark:text-slate-400 text-gray-600">{{ item.position }}</td>
              <td class="px-4 py-3.5 dark:text-slate-400 text-gray-600 whitespace-nowrap">{{ item.startDate }}</td>
              <td class="px-4 py-3.5 dark:text-slate-400 text-gray-600 whitespace-nowrap">{{ item.endDate }}</td>
              <td class="px-4 py-3.5">
                <span
        ...
        <div v-else class="px-4 py-12 text-center">
          <div class="flex flex-col items-center">
            ...
            <h3 class="mt-3 text-sm font-semibold dark:text-slate-200 text-gray-700">No internship information found</h3>
