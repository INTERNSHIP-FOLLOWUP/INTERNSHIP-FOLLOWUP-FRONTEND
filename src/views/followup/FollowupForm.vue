...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-3 sm:p-4">
    <div
    ...
    >
      ...
      <form @submit.prevent="submit" class="space-y-5">
        ...
        <div>
          ...
          <p v-if="companiesError" class="text-red-600 text-xs mt-1.5">{{ companiesError }}</p>
          <p v-if="errors.company_id" class="text-red-600 text-xs mt-1.5">
            {{ errors.company_id }}
          </p>
        </div>
        ...
        <div>
          <div class="flex items-center justify-between mb-1.5">
            ...
            <span
              ...
              :class="{ 'text-amber-500': notesLength > 0 }"
              v-if="form.notes"
            >
              {{ notesLength }}
            </span>
        ...
        <div>
          ...
          <transition name="error-slide">
            <p v-if="errors.status" class="text-red-600 text-xs mt-1.5 flex items-center gap-1">
              <svg
                class="h-3.5 w-3.5 shrink-0"
                fill="none"
                stroke="currentColor"
            ...
            >
              ...
              >
                <path
                  ...
                  stroke-linejoin="round"
                  stroke-width="2"
                  d="M4 16v4h4l10.5-10.5-4-4L4 16zm14.5-9.5l-1.5 1.5 4 4 1.5-1.5-4-4z"
                />
              </svg>
...
const form = reactive<FollowupPayload & { status: FollowupStatus }>({
  ...
  status: props.followup?.status ?? 'Scheduled',
})
...
async function fetchCompanies() {
  ...
  try {
    ...
    companies.value = (res.data.data || res.data || []).map((c: any) => ({
      id: c.id,
