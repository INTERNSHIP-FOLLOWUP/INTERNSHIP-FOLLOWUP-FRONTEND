  ...
  <div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <div
        ...
        :hover="{ scale: 1.02, y: -4, transition: { type: 'spring', stiffness: 400, damping: 15 } }"
        class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md"
      >
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-gray-500">{{ stat.label }}</p>
            <p class="mt-1 text-2xl font-bold text-gray-900">{{ stat.displayValue }}</p>
          </div>
        ...
        <p class="mt-2 text-xs" :class="stat.trend.startsWith('+') ? 'text-green-600' : 'text-gray-400'">
          {{ stat.trend }}
        </p>
      </div>
    </div>
