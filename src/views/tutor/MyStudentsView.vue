<template>
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight text-[#111827] dark:text-slate-100 dark:text-slate-100">My Students</h1>
        <p class="mt-1 text-sm text-[#6B7280] dark:text-slate-400 dark:text-slate-400">Manage and monitor your assigned students.</p>
      </div>
      <button
        type="button"
        @click="refresh"
        :disabled="store.loading"
        class="inline-flex items-center gap-2 rounded-xl border border-[#E5E7EB] bg-white px-4 py-2.5 text-sm font-medium text-[#6B7280] dark:text-slate-400 transition-all dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 hover:border-[#4F46E5]/20 hover:bg-[#4F46E5]/5 hover:text-[#4F46E5] disabled:opacity-50"
      >
        <svg
          class="h-4 w-4"
      ...
      <div class="flex flex-wrap items-center gap-3">
        ...
        <div class="relative min-w-0 flex-1 basis-[260px]">
          ...
          <input
            ...
            class="h-10 w-full rounded-xl border border-[#E5E7EB] bg-white py-2 pl-10 pr-10 text-sm text-[#111827] dark:text-slate-100 placeholder-[#9CA3AF] transition-all focus:border-[#4F46E5] focus:outline-none focus:ring-2 focus:ring-[#4F46E5]/10"
          />
          <button
            v-if="localSearch.length > 0"
            type="button"
            @click="clearSearch"
            class="absolute inset-y-0 right-0 flex items-center pr-3 text-[#9CA3AF] dark:text-slate-500 transition-colors hover:text-[#6B7280] dark:text-slate-400"
        ...
        <select
          ...
          class="h-10 min-w-[150px] rounded-xl border border-[#E5E7EB] bg-white px-3.5 pr-8 text-sm text-[#111827] dark:text-slate-100 transition-all focus:border-[#4F46E5] focus:outline-none focus:ring-2 focus:ring-[#4F46E5]/10 appearance-none"
        >
        ...
        <select
          ...
          @change="onFilterChange"
          class="h-10 min-w-[170px] rounded-xl border border-[#E5E7EB] bg-white px-3.5 pr-8 text-sm text-[#111827] dark:text-slate-100 transition-all focus:border-[#4F46E5] focus:outline-none focus:ring-2 focus:ring-[#4F46E5]/10 appearance-none"
        >
        ...
        <button
          ...
          class="inline-flex h-10 items-center gap-1.5 rounded-xl border border-[#E5E7EB] bg-white px-4 text-sm font-medium text-[#6B7280] dark:text-slate-400 transition-all hover:border-[#EF4444]/30 hover:bg-[#FEF2F2] hover:text-[#EF4444] disabled:opacity-40"
    ...
    <div class="overflow-hidden rounded-xl border border-[#E5E7EB] bg-white shadow-sm">
      ...
      <div v-if="store.loading" class="divide-y divide-[#E5E7EB]/60">
        ...
        >
          ...
          <div class="flex-1 space-y-2">
            ...
            <div class="h-3 w-1/6 rounded bg-[#F3F4F6] dark:bg-slate-600" />
          </div>
      ...
      <template v-else>
        <!-- Table Header -->
        <div class="border-b border-[#E5E7EB] bg-[#F9FAFB] dark:bg-slate-700">
        ...
        <div class="divide-y divide-[#E5E7EB]/60">
          ...
          >
            ...
            <div class="w-[280px] flex items-center gap-3 min-w-0">
              ...
              <div class="min-w-0">
                <p class="text-sm font-semibold text-[#111827] dark:text-slate-100 truncate">{{ student.name }}</p>
                <p class="text-xs text-[#4F46E5] font-medium">{{ student.student_code || '—' }}</p>
                <p class="text-xs text-[#9CA3AF] dark:text-slate-500 truncate">{{ student.email }}</p>
              </div>
            </div>
            ...
            <div class="w-[180px] flex items-center gap-2 min-w-0">
              ...
              <div class="min-w-0">
                <p class="text-sm font-medium text-[#111827] dark:text-slate-100 truncate">{{ student.company_name || '—' }}</p>
                <p class="text-xs text-[#9CA3AF] dark:text-slate-500 truncate">{{ student.position || '' }}</p>
              </div>
            </div>
            ...
            <div class="flex-1 flex items-center justify-end gap-1">
              <button
                ...
                class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-[#9CA3AF] dark:text-slate-500 transition-all hover:bg-[#F3F4F6] dark:bg-slate-600 hover:text-[#4F46E5]"
                title="View Profile"
              >
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="relative">
                <button
                  ...
                  class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-[#9CA3AF] dark:text-slate-500 transition-all hover:bg-[#F3F4F6] dark:bg-slate-600 hover:text-[#6B7280] dark:text-slate-400"
                  title="More actions"
                >
                  <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex items-center justify-between border-t border-[#E5E7EB] px-6 py-3">
          ...
          <nav class="flex items-center gap-1" aria-label="Pagination">
            <button
              ...
              :class="store.pagination.current_page <= 1 ? 'text-[#D1D5DB] cursor-not-allowed' : 'text-[#6B7280] dark:text-slate-400 hover:bg-[#F3F4F6] dark:bg-slate-600 hover:text-[#111827] dark:text-slate-100'"
            >
              <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <template v-for="(page, idx) in visiblePages" :key="idx">
              <span v-if="page === '...'" class="inline-flex h-8 w-8 items-center justify-center text-xs text-[#9CA3AF] dark:text-slate-500 select-none">…</span>
              <button
                v-else
                ...
                :class="page === store.pagination.current_page ? 'bg-[#EEF2FF] text-[#4F46E5]' : 'text-[#6B7280] dark:text-slate-400 hover:bg-[#F3F4F6] dark:bg-slate-600 hover:text-[#111827] dark:text-slate-100'"
              >
                {{ page }}
              </button>
            </template>
            ...
            <button
              ...
              @click="onPageChange(store.pagination.current_page + 1)"
              class="inline-flex h-8 w-8 items-center justify-center rounded-lg text-sm font-medium transition-all"
              :class="store.pagination.current_page >= store.pagination.last_page ? 'text-[#D1D5DB] cursor-not-allowed' : 'text-[#6B7280] dark:text-slate-400 hover:bg-[#F3F4F6] dark:bg-slate-600 hover:text-[#111827] dark:text-slate-100'"
            >
              <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="flex items-center gap-2">
            ...
            <select
              ...
              class="h-8 rounded-lg border border-[#E5E7EB] bg-white px-2 text-sm text-[#111827] dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-[#4F46E5]/10"
            >
...
function statusClass(status?: string): string {
  switch (status) {
    ...
    default: return 'bg-[#F3F4F6] dark:bg-slate-600 text-[#6B7280] dark:text-slate-400'
  }
}
