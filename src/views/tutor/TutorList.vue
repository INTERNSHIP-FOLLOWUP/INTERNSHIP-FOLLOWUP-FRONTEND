              ...
              <tr v-for="tutor in store.tutors" :key="tutor.id" class="transition-colors hover:bg-slate-50/50">
                ...
                <td class="whitespace-nowrap px-6 py-4 text-right">
                  <div class="flex items-center justify-end gap-1">
                    <router-link :to="`/admin/tutors/${tutor.id}/edit`" class="rounded-lg px-3 py-1.5 text-xs font-bold text-primary-600 transition-all hover:bg-primary-50 hover:text-primary-800">
                      Edit
                    </router-link>
                    <button @click="onDelete(tutor.id)" class="rounded-lg px-3 py-1.5 text-xs font-bold text-rose-600 transition-all hover:bg-rose-50 hover:text-rose-800">
                      Delete
                    </button>
                  </div>
                </td>
      ...
      <div v-else-if="store.tutors.length > 0" class="divide-y divide-slate-100 md:hidden">
        <div v-for="tutor in store.tutors" :key="tutor.id" class="p-4 transition-colors hover:bg-slate-50/50">
          ...
          <div class="mt-3 flex items-center gap-2">
            ...
            <button @click="onDelete(tutor.id)" class="flex-1 rounded-lg border border-rose-200 px-3 py-2 text-xs font-semibold text-rose-600 transition-colors hover:bg-rose-50">
              Delete
            </button>
          </div>
        </div>
