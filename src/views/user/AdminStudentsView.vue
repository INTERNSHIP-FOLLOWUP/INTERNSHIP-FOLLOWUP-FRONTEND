<template>
  <div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">{{ $t('users.studentsTitle') }}</h1>
        <p class="mt-1 text-sm text-slate-500">{{ totalStudents }} {{ $t('users.registeredStudents') }}</p>
      </div>
    ...
    <div class="flex flex-wrap items-center gap-3">
      <div class="relative min-w-0 flex-1 basis-[200px]">
        ...
        <input v-model="searchQuery" type="text" :placeholder="$t('users.searchStudents')"
      ...
      <select v-model="sortOrder"
        ...
        <option value="name_asc">{{ $t('users.sortNameAsc') }}</option>
        <option value="name_desc">{{ $t('users.sortNameDesc') }}</option>
    ...
    <div class="rounded-xl border border-slate-200/80 bg-white shadow-sm">
          ...
          <table class="w-full text-left text-sm">
            <thead>
              <tr class="border-b border-slate-100 bg-slate-50/50 text-xs font-semibold uppercase tracking-wider text-slate-400">
                ...
                <th class="px-6 py-3.5 font-medium">{{ $t('users.studentCodeCol') }}</th>
              ...
              <tr v-for="student in students" :key="student.id"
                ...
                <td class="whitespace-nowrap px-6 py-4">
                  <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-bold"
                    ...
                    {{ student.deleted_at ? $t('users.deactivated') : $t('common.active') }}
                  </span>
                </td>
                <td class="whitespace-nowrap px-6 py-4 text-right">
                  <div class="flex items-center justify-end gap-1">
                    <button type="button" @click.stop="goToProfile(student.id)" :title="$t('users.viewProfile')"
                    ...
                    <button type="button" @click.stop="editStudent(student.id)" :title="$t('users.editStudent')"
                    ...
                    <button type="button" @click.stop="deleteStudent(student)" :title="$t('users.deleteStudentBtn')"
      ...
      <div v-else class="flex flex-col items-center justify-center px-6 py-16 text-center">
        <h3 class="text-base font-bold text-slate-900">{{ $t('users.noStudentsFound') }}</h3>
        <p class="mt-1 text-sm text-slate-500">{{ $t('users.noRegisteredStudentsYet') }}</p>
      </div>
    </div>
      ...
      <div v-if="selectedIds.size > 0"
        ...
        <div class="flex items-center gap-3 rounded-2xl border border-rose-200 bg-white px-5 py-3 shadow-2xl shadow-rose-100 ring-1 ring-rose-100">
          ...
          <span class="text-sm font-semibold text-slate-700">
            {{ selectedIds.size }} {{ $t('users.studentsSelected') }}
          </span>
...
async function confirmBulkDelete() {
  ...
  try {
    ...
    toast.success($t_script('users.studentsBulkDeleted', { count: ids.length }))
...
function onStudentSaved() {
  closeFormModal()
  fetchStudents()
  toast.success($t_script('users.studentSavedToast'))
}
...
async function handleConfirmAction() {
  ...
  await confirmAsyncFn(async () => {
    if (type === 'delete') {
      ...
      toast.success($t_script('users.studentDeletedNameToast', { name: `${student.first_name} ${student.last_name}` }))
    }
